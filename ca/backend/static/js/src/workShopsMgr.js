var workShopsMgr=(function(config,functions){
    ...
    function createTable(){
        ...
        var ownTable=$("#myTable").dataTable({
            "bServerSide": true,
            "sAjaxSource": config.ajaxUrls.workShopsGetAll,
            "bInfo":true,
            "bLengthChange": false,
            "bFilter": false,
            "bSort":false,
            ...
            "aoColumns": [
                ...
                { "mDataProp": "opt",
                    "fnRender":function(oObj){
                        var string= '<a href="manager/workshops/'+oObj.aData.id+'/update">修改</a>&nbsp;' +
    ...
    return {
        ...
        delete:function(id){
            ...
            var me=this;
            $.ajax({
                url:config.ajaxUrls.workShopDelete.replace(":id",id),
                type:"post",
                dataType:"json",
                success:function(response){
                    if(response.success){
        ...
        getAllThinsBinded:function(id){
            ...
            functions.showLoading();
            $.ajax({
                url:config.ajaxUrls.workShopBinds.replace(":id",id),
                type:"get",
                dataType:"json",
...
$(document).ready(function(){
    ...
    workShopsMgr.createTable();

    $("#searchBtn").click(function(e){
        workShopsMgr.tableRedraw();
    });
    ...
    $("#bindDesigners").submit(function(){
        workShopsMgr.bindDesignersFormSubmit($(this));
        return false;
    });
    ...
    $("#myTable").on("click","a.delete",function(){
        if(confirm(config.messages.confirmDelete)){
            workShopsMgr.delete($(this).attr("href"));
        }
        return false;
    }).on("click","a.bindData",function(){
            workShopsMgr.bindData($(this).attr("href"));
            return false;
        });
});
